...
export default function ElegantDashboard() {
  const modules = [
    ...
    { title: 'المحفظة', icon: <Video size={28} className="text-cyan-400" />, link: '/ar/library' },
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {modules.map((mod, i) => (
            <Link key={i} href={mod.link}>
              <div className="bg-[#0A0A0F] border border-white/5 p-8 rounded-3xl hover:border-indigo-500/30 transition-all flex flex-col items-center justify-center text-center gap-4">
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(fixed_code)
print("✅ تم إصلاح هيكل الداشبورد بنجاح!")
