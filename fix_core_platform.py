...
# 1. إصلاح الواجهة الرئيسية (Navbar + Academy Image)
...
export default function UltimateLandingPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 font-sans ${isDarkMode ? 'bg-[#05050A] text-slate-50' : 'bg-slate-50 text-slate-900'} selection:bg-indigo-500/30 overflow-x-hidden`} dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* شريط الملاحة الموحد (تم استعادته ودمجه) */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 backdrop-blur-xl border-b ${isDarkMode ? 'bg-[#05050A]/80 border-white/5' : 'bg-white/80 border-slate-200 shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <button onClick={toggleLanguage} className={`font-bold text-xs md:text-sm px-2 py-1 md:px-3 md:py-1.5 rounded-lg transition-colors border ${isDarkMode ? 'border-white/10 hover:bg-slate-800 text-slate-300' : 'border-slate-200 hover:bg-slate-200 text-slate-700'}`}>
              {isAr ? 'English' : 'عربي'}
            </button>
            <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-1.5 md:p-2 rounded-full transition-colors border ${isDarkMode ? 'border-white/10 bg-slate-800 text-amber-400 hover:bg-slate-700' : 'border-slate-200 bg-slate-200 text-indigo-600 hover:bg-slate-300'}`}>
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center pt-24 pb-12 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className={`absolute inset-0 z-10 bg-gradient-to-b ${isDarkMode ? 'from-[#05050A]/70 via-[#05050A]/90 to-[#05050A]' : 'from-slate-50/80 via-slate-50/90 to-slate-50'}`}></div>
          <Image src="/images/monteerly/monteerly_04_hero_header_marketing_banner.png" alt="Hero" fill priority className="object-cover opacity-40 scale-105 mix-blend-overlay" />
        </div>
      ...
      <section className={`py-6 md:py-8 border-y ${isDarkMode ? 'bg-[#0A0A0F] border-white/5' : 'bg-white border-slate-200'}`}>
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
            <span className="text-xl font-black tracking-widest">PAYMOB</span>
            <span className="text-xl font-black tracking-widest italic">VISA</span>
            <span className="text-xl font-black tracking-widest">MasterCard</span>
            <span className="text-xl font-black tracking-widest text-blue-500 italic">PayPal</span>
          </div>
        </div>
      </section>
      ...
      <section className={`py-16 md:py-32 px-4 relative ${isDarkMode ? 'bg-[#030305]' : 'bg-slate-100'}`} id="library">
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <div className={`col-span-1 lg:col-span-2 p-6 md:p-12 rounded-3xl md:rounded-[2.5rem] border overflow-hidden relative group flex flex-col justify-between min-h-[350px] md:min-h-[450px] ${isDarkMode ? 'bg-[#0A0A0F] border-white/10' : 'bg-white border-slate-200'}`}>
              ...
              <div className="relative z-10 -mx-6 -mb-6 md:-mx-12 md:-mb-12 mt-auto">
                <Image src="/images/monteerly/monteerly_05_security_digital_lock_cyber.png" alt="Escrow" width={600} height={400} className="w-full h-40 md:h-56 object-cover object-top opacity-80" />
              </div>
            </div>
            ...
            <div className={`col-span-1 lg:col-span-2 p-6 md:p-12 rounded-3xl md:rounded-[2.5rem] border overflow-hidden relative group flex flex-col justify-between min-h-[350px] md:min-h-[450px] ${isDarkMode ? 'bg-[#0A0A0F] border-white/10' : 'bg-white border-slate-200'}`}>
              ...
              <div className="relative z-10 -mx-6 -mb-6 md:-mx-12 md:-mb-12 mt-auto">
                <Image src="/images/monteerly/monteerly_09_ai_performance_advisor_hologram.png" alt="AI" width={600} height={400} className="w-full h-40 md:h-56 object-cover opacity-80" />
              </div>
            </div>

            <div className={`col-span-1 lg:col-span-2 p-6 md:p-12 rounded-3xl md:rounded-[2.5rem] border overflow-hidden relative group flex flex-col justify-between min-h-[350px] md:min-h-[450px] ${isDarkMode ? 'bg-[#0A0A0F] border-white/10' : 'bg-white border-slate-200'}`}>
              <div className="relative z-20 mb-6">
                <Video className="w-10 h-10 md:w-14 md:h-14 text-pink-400 mb-4 md:mb-6" />
              ...
              <div className="relative z-10 -mx-6 -mb-6 md:-mx-12 md:-mb-12 mt-auto">
                <Image src="/images/monteerly/monteerly_23_photographer_library_heritage_cinematic.png" alt="Library" width={600} height={400} className="w-full h-40 md:h-56 object-cover opacity-80" />
              </div>
            </div>

            {/* تم الإصلاح: صورة الأكاديمية والمجتمع تم تحديثها بصورة ملائمة */}
            <div className={`col-span-1 lg:col-span-2 p-6 md:p-12 rounded-3xl md:rounded-[2.5rem] border overflow-hidden relative group flex flex-col justify-between min-h-[350px] md:min-h-[450px] ${isDarkMode ? 'bg-[#0A0A0F] border-white/10' : 'bg-white border-slate-200'}`}>
              <div className="relative z-20 mb-6">
                <Users className="w-10 h-10 md:w-14 md:h-14 text-amber-400 mb-4 md:mb-6" />
              ...
              <div className="relative z-10 -mx-6 -mb-6 md:-mx-12 md:-mb-12 mt-auto">
                <Image src="/images/monteerly/monteerly_16_editor_arab_thobe_collaboration.png" alt="Academy" width={600} height={400} className="w-full h-40 md:h-56 object-cover object-top opacity-80" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
            <div className={`p-8 rounded-3xl border text-center ${isDarkMode ? 'bg-[#0A0A0F] border-white/10' : 'bg-white border-slate-200 shadow-lg'}`}>
              ...
              <p className="text-4xl font-black mb-6 text-indigo-500">{isAr ? 'مجاني' : 'Free'}</p>
            ...
            <div className={`p-8 rounded-3xl border text-center relative overflow-hidden ${isDarkMode ? 'bg-indigo-900/20 border-indigo-500/30' : 'bg-indigo-50 border-indigo-200 shadow-xl'}`}>
              <div className="absolute top-0 right-0 bg-indigo-500 text-white text-xs font-bold px-4 py-1 rounded-bl-xl">{isAr ? 'الأكثر طلباً' : 'Popular'}</div>
              <h3 className="text-2xl font-bold mb-2">Pro & Enterprise</h3>
              <p className="text-4xl font-black mb-6 text-indigo-500">{isAr ? 'مخصص' : 'Custom'}</p>
              <ul className="text-right space-y-4 mb-8">
                <li className="flex gap-2 items-center"><CheckCircle2 className="w-5 h-5 text-indigo-500"/> {isAr ? 'مساحات تخزين غير محدودة (R2)' : 'Unlimited R2 Storage'}</li>
                <li className="flex gap-2 items-center"><CheckCircle2 className="w-5 h-5 text-indigo-500"/> {isAr ? 'عمولات مخفضة وفواتير ضريبية' : 'Reduced Fees & Tax Invoices'}</li>
              </ul>
              <Link href={`/${locale}/auth`} className="block w-full py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-500 transition-all shadow-lg">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          <div className="col-span-1 md:col-span-2">
             <div className="flex items-center gap-3 mb-4 opacity-90">
                ...
                <span className="font-black text-xl tracking-tight">Monteerly <span className="text-indigo-500">OS</span></span>
             </div>
          ...
          <div>
            ...
            <ul className={`space-y-2 text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
              <li><Link href="#ecosystem" className="hover:text-indigo-500">{isAr ? 'السوق' : 'Marketplace'}</Link></li>
              <li><Link href="#library" className="hover:text-indigo-500">{isAr ? 'المكتبة' : 'Library'}</Link></li>
              <li><Link href="#pricing" className="hover:text-indigo-500">{isAr ? 'الأسعار' : 'Pricing'}</Link></li>
            </ul>
          </div>
          <div>
            <h4 className="font-bold mb-4">{isAr ? 'قانوني' : 'Legal'}</h4>
            <ul className={`space-y-2 text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
              <li><Link href="#" className="hover:text-indigo-500">{isAr ? 'شروط الاستخدام' : 'Terms of Service'}</Link></li>
              <li><Link href="#" className="hover:text-indigo-500">{isAr ? 'سياسة الخصوصية' : 'Privacy Policy'}</Link></li>
            </ul>
          </div>
        </div>
...
# 2. جعل اللغة العربية هي الافتراضية في ملف Middleware
middleware_path = "src/middleware.ts"
if os.path.exists(middleware_path):
    with open(middleware_path, "r", encoding="utf-8") as f:
        mw_content = f.read()
    
    # تبديل اللغة الافتراضية من en إلى ar برمجياً
    mw_content = mw_content.replace("defaultLocale: 'en'", "defaultLocale: 'ar'")
    mw_content = mw_content.replace('defaultLocale: "en"', 'defaultLocale: "ar"')
    
    with open(middleware_path, "w", encoding="utf-8") as f:
        f.write(mw_content)
    print("✅ تم تعديل Middleware لتصبح العربية هي اللغة الافتراضية!")

print("✅ تم استعادة أزرار اللغة والليل/النهار.")
print("✅ تم استبدال صورة الأكاديمية بصورة معبرة عن المجتمع العربي.")
