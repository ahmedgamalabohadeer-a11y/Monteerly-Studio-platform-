...
# ==========================================
# 1. القضاء على تمرد اللغة الإنجليزية (Middleware Patch)
# ==========================================
...
if os.path.exists(middleware_path):
    ...
    # تحويل اللغة الافتراضية إلى العربية
    mw_content = re.sub(r"defaultLocale:\s*['\"]en['\"]", "defaultLocale: 'ar'", mw_content)
    
    # إجبار النظام على تجاهل لغة متصفح الزائر (Locale Detection Disable)
    if "localeDetection" not in mw_content:
        mw_content = re.sub(r"(defaultLocale:\s*['\"]ar['\"],?)", r"\1\n  localeDetection: false,", mw_content)
    
    with open(middleware_path, "w", encoding="utf-8") as f:
        f.write(mw_content)
    print("✅ تم ترويض Middleware: العربية الآن هي القائد الافتراضي (تم تعطيل localeDetection).")


# ==========================================
# 2. إصلاح الصورة المقطوعة والتباين (Landing Page UI Polish)
# ==========================================
...
print("✅ تم صقل الواجهة بامتياز وإصلاح صورة الأكاديمية ووضع النهار (Light Mode).")
