...
export default function FreelancerProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-[#05050A] text-slate-50 p-4 md:p-8 font-sans relative" dir="rtl">
      <button
        onClick={() => router.back()}
        className="absolute top-8 right-8 text-slate-400 hover:text-white flex items-center gap-2 transition-colors z-10"
      >
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 px-8 py-3 rounded-xl font-black transition-all shadow-[0_0_20px_rgba(79,70,229,0.3)] text-center flex items-center gap-2"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 bg-[#0A0A0F] border border-white/5 p-8 rounded-[2rem]">
            <h2 className="text-xl font-black mb-6 flex items-center gap-2">
              <Layers className="text-indigo-500" /> معرض الأعمال (Portfolio)
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="aspect-video bg-[#12121A] rounded-2xl border border-white/5 flex flex-col items-center justify-center hover:border-indigo-500/50 cursor-pointer transition-all group overflow-hidden relative"
                >
                  <Image
                    src={`https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?q=80&w=500&auto=format&fit=crop&sig=${i}`}
                    alt={`Portfolio ${i}`}
                    fill
                    className="object-cover opacity-50 group-hover:opacity-100 transition-opacity"
                  />
                  <div className="absolute inset-0 bg-black/40 group-hover:bg-transparent transition-colors z-10"></div>
                  <span className="text-white font-bold z-20 group-hover:scale-110 transition-transform bg-black/50 px-3 py-1 rounded-full backdrop-blur-sm">
                    المشروع {i}
                  </span>
                </div>
              ))}
