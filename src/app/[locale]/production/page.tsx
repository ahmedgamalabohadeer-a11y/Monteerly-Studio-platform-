...
export default function ProductionPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      if (data) {
        setProjects(data as ProductionProject[]);
      }
        ...
        {loading ? (
          ...
          <div className="grid gap-6">
            {projects.map((project) => (
              <div
                key={project.id}
                className="group bg-slate-900 border border-slate-800 hover:border-brand-primary/50 transition-all p-6 rounded-2xl flex justify-between items-center shadow-lg"
              >
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">
                      ...
                      <span className="text-white font-semibold">
                        {project.legal_contracts?.client_name || 'غير متوفر'}
                      </span>
                ...
                <div className="flex flex-col items-end gap-3">
                  ...
                  <Link
                    href={`/ar/production/${project.id}`}
                    className="text-xs text-brand-primary hover:text-white font-bold flex items-center gap-1 transition-colors px-3 py-1 rounded-lg hover:bg-brand-primary/20"
                  >
