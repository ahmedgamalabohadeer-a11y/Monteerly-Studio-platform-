  ...
  return (
    <div className="p-8 bg-[#F8FAFC] min-h-screen font-sans" dir="rtl">
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-900">الترسانة القانونية | Legal OS</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-50 p-6 rounded-[2rem] shadow-inner border border-slate-200 h-fit">
          ...
          <form action={addContractRevision} className="space-y-4">
            <select name="contract_id" required className="w-full p-3 bg-white rounded-xl text-sm outline-none border border-slate-200">
              <option value="">-- اختر العقد المراد تعديله --</option>
              {contracts?.map(c => (
                <option key={c.id} value={c.id}>{c.title} (الحالي: V{c.current_version})</option>
              ))}
            </select>
            <input name="changes_summary" required placeholder="ملخص التعديلات القانونية" className="w-full p-3 bg-white rounded-xl text-sm outline-none border border-slate-200" />
             ...
             {revisions?.map((rev, i) => (
               <div key={i} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                 <div className="flex justify-between">
                   <span className="text-xs font-bold text-slate-800">{rev.contracts?.title}</span>
        ...
        <div className="md:col-span-3 bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
          <h3 className="font-bold mb-6 text-slate-800">سجل العقود السيادي</h3>
            ...
            {contracts?.map((contract) => (
              <div key={contract.id} className="flex justify-between items-center p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                ...
                <div className="text-left">
                  <div className="text-xs font-black text-slate-900 bg-slate-200 px-3 py-1 rounded-full inline-block mb-1">
