...
export default async function HRPage() {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen font-sans" dir="rtl">
      <h1 className="text-2xl font-bold mb-6 text-slate-800">إدارة الموارد البشرية | Monteerly HR</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h3 className="font-semibold mb-4 text-slate-700">سجل الموظفين الحالي</h3>
                ...
                {employees && employees.length > 0 ? (
                  employees.map((emp) => (
                    <tr key={emp.id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                      <td className="py-3 font-medium text-slate-800">{emp.full_name}</td>
