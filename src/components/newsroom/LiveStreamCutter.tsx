...
import { Square, Scissors, Share2, Clock } from 'lucide-react';
      ...
      <div className="lg:col-span-2 flex flex-col bg-slate-900 border border-white/10 rounded-xl overflow-hidden">
        <div className="relative flex-1 bg-black">
          <Image
            ...
            alt="بث مباشر قيد القص والتحرير"
            fill
            priority
          ...
          <div className="absolute top-4 left-4 flex gap-2">
            ...
            <span className="bg-black/60 text-white px-2 py-1 rounded text-xs font-mono">RTMP INGEST: source_01</span>
          </div>
            ...
            <div key={clip.id} className="bg-black/30 border border-white/5 p-3 rounded-lg hover:bg-white/5 transition-colors group">
              ...
              <div className="flex justify-between items-center text-xs text-slate-400">
                <span className="flex items-center gap-1">
                  <Clock size={10} /> {clip.time}
                </span>

                <button
                  type="button"
                  aria-label={`نشر سريع للمقطع ${clip.title}`}
                  className="text-indigo-400 hover:text-white flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
                >
