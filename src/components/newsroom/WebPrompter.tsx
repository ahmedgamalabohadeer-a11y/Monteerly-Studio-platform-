...
import { Play, Pause, Settings, FlipHorizontal, Type, RefreshCcw } from 'lucide-react';
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col md:flex-row gap-6">
       ...
       <div className="w-full md:w-64 bg-slate-900 border border-white/10 rounded-xl p-6 flex flex-col gap-6">
          ...
          <div className="flex gap-2">
             <Button 
               ...
               variant="outline" 
               className={`flex-1 ${mirror ? 'bg-indigo-600 border-transparent text-white' : 'border-white/10 text-slate-300'}`}
             >
          ...
          <div className="mt-auto space-y-2">
             <Button 
               ...
               className={`w-full font-bold py-6 text-lg ${isPlaying ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'} text-white`}
             >
       ...
       <div className="flex-1 bg-black rounded-xl border-4 border-slate-800 overflow-hidden relative">
          ...
          <div 
             ref={scrollRef}
             className={`w-full h-full overflow-y-auto px-12 py-[40vh] text-white font-bold leading-relaxed text-center scrollbar-hide ${mirror ? 'scale-x-[-1]' : ''}`}
