...
export function WelcomeTour() {
  ...
  const roles = [
    { id: 'student', icon: GraduationCap, title: 'طالب / مبتدئ', desc: 'أريد تعلم المونتاج وبناء معرض أعمالي.' },
    { id: 'freelancer', icon: User, title: 'مستقل (Freelancer)', desc: 'أريد بيع خدماتي وإدارة عملائي.' },
    { id: 'agency', icon: Briefcase, title: 'شركة إنتاج', desc: 'أريد إدارة فريق وعقود ضخمة.' },
  ];

  if (step === 3) return null; // Tour finished
          ...
          <div className="p-8">
             {step === 1 && (
                <div className="text-center">
                   ...
                   <InteractiveButton onClick={() => setStep(2)} className="w-48 mx-auto" icon={<ArrowRight size={18}/>}>
                      ابدأ التخصيص
                   </InteractiveButton>
                </div>
             )}
                ...
                <div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                      {roles.map((r) => (
                        ...
                        >
                            <r.icon size={32} className={`mb-4 ${role === r.id ? 'text-indigo-400' : 'text-slate-400'}`} />
                            <h3 className="font-bold text-white mb-2">{r.title}</h3>
                            <p className="text-xs text-slate-400 leading-relaxed">{r.desc}</p>
                            {role === r.id && <div className="mt-4 flex justify-end"><Check size={16} className="text-indigo-400"/></div>}
                         </div>
                      ))}
                   </div>
