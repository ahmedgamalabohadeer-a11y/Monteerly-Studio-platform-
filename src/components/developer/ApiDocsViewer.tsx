...
export function ApiDocsViewer() {
  ...
  const codeSnippets = {
    curl: `curl -X POST [https://api.monteerly.com/v1/plugins](https://api.monteerly.com/v1/plugins) \\
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="bg-[#1e1e1e] border border-white/10 rounded-xl overflow-hidden shadow-2xl">
          <div className="flex items-center justify-between p-2 bg-[#252526] border-b border-white/5">
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1 rounded text-xs font-bold ${
                  activeLang === 'js' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Node.js
              </button>
            ...
            <div className="flex gap-2">
              <InteractiveButton size="sm" icon={<Play size={12} />}>
                تشغيل التجربة
              </InteractiveButton>
            </div>
          </div>
          ...
          <div className="p-4 overflow-x-auto">
            <pre className="font-mono text-sm text-blue-300">{codeSnippets[activeLang]}</pre>
          </div>
