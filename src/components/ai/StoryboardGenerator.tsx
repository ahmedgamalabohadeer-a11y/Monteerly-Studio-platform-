  ...
  return (
    <div className="bg-slate-900 border border-white/10 rounded-xl p-6 h-full flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-4 flex-1 overflow-y-auto min-h-[300px]">
        {images.length > 0 ? (
          images.map((img, i) => (
            <div key={i} className="relative group rounded-lg overflow-hidden border border-white/10 min-h-[140px]">
              <Image
                src={img}
                alt={`مشهد مولد رقم ${i + 1}`}
                fill
                sizes="(max-width: 768px) 50vw, 25vw"
                className="object-cover"
              />
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <button className="p-2 bg-white/20 rounded-full hover:bg-white/40 text-white">
                  <Download size={16} />
                </button>
                <button className="p-2 bg-white/20 rounded-full hover:bg-white/40 text-white">
                  <ImageIcon size={16} />
