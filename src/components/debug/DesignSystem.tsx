    ...
    <div className="space-y-12">
      ...
      <section>
        <h3 className="text-xl font-bold text-white mb-4 border-b border-white/10 pb-2">
          2. التنبيهات (Feedback System)
        </h3>
        <div className="flex flex-wrap gap-4">
          <Button
            onClick={() => addToast('success', 'تم حفظ التغييرات بنجاح!')}
            className="bg-green-900/50 border border-green-500/50 text-green-400"
          >
            Trigger Success
          </Button>
          <Button
            onClick={() => addToast('error', 'فشل الاتصال بالسيرفر.')}
            className="bg-red-900/50 border border-red-500/50 text-red-400"
          >
            Trigger Error
          </Button>
          <Button
            onClick={() => addToast('warning', 'مساحة التخزين قاربت على الامتلاء.')}
            className="bg-yellow-900/50 border border-yellow-500/50 text-yellow-400"
          >
            Trigger Warning
          </Button>
          <Button
            onClick={() => addToast('info', 'لديك رسالة جديدة من سارة.')}
            className="bg-blue-900/50 border border-blue-500/50 text-blue-400"
          >
            Trigger Info
          </Button>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="p-4 rounded-xl bg-slate-900 text-slate-400 text-center border border-white/10">
            Surface Dark
          </div>
