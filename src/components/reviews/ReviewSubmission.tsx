...
export function ReviewSubmission({ freelancerName }: { freelancerName: string }) {
  ...
  const handleRate = (criteria: keyof typeof ratings, score: number) => {
    setRatings((prev) => ({ ...prev, [criteria]: score }));
  };
    ...
    <div className="max-w-lg mx-auto bg-card border border-border rounded-2xl p-8 shadow-sm text-center">
      ...
      <div className="space-y-6 mb-8 text-left">
        <RatingCriteria
          ...
          onChange={(v) => handleRate('quality', v)}
        />
        <RatingCriteria
          label="التواصل والمتابعة"
          value={ratings.communication}
          onChange={(v) => handleRate('communication', v)}
        />
        <RatingCriteria
          label="الالتزام بالمواعيد"
          value={ratings.time}
          onChange={(v) => handleRate('time', v)}
        />
      </div>
      ...
      <div className="flex gap-3">
        ...
        <Button variant="primary" className="flex-1" icon={<ThumbsUp size={16} />}>
          نشر التقييم
        </Button>
      </div>
    </div>
...
function RatingCriteria({ label, value, onChange }: RatingCriteriaProps) {
  return (
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            className={`transition-transform hover:scale-110 ${
              value >= star ? 'text-yellow-500' : 'text-muted-foreground/30'
            }`}
          >
