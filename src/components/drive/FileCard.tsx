...
export function FileCard({ name, type, size, date, thumbnail }: FileProps) {
  ...
  return (
    <div className="group bg-slate-900 border border-white/10 rounded-xl p-4 hover:bg-white/5 hover:border-indigo-500/50 transition-all cursor-pointer relative">
      <div className="aspect-square bg-black/40 rounded-lg mb-4 flex items-center justify-center overflow-hidden border border-white/5 relative">
        {thumbnail ? (
          <Image
            src={thumbnail}
            alt={name}
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
          <button
            className="p-2 bg-white text-black rounded-full hover:bg-indigo-500 hover:text-white transition-colors"
            title="Download"
          >
            <Download size={16} />
          </button>
      ...
      <div className="flex justify-between items-start">
        <div className="overflow-hidden">
          <h4 className="font-bold text-white text-sm truncate mb-1" title={name}>
            {name}
          </h4>
