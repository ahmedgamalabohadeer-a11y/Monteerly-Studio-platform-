...
import { Wand2, Music, Type, Video, Download, Heart, MessageCircle, Share2 } from 'lucide-react';
import { Button } from '@/components/ui/Button';

type ReelStyle = 'fast' | 'minimal' | 'cinematic';

export function AutoReelsGen() {
  const [step, setStep] = useState(1);
  const [topic, setTopic] = useState('');
  const [loading, setLoading] = useState(false);
  const [selectedStyle, setSelectedStyle] = useState<ReelStyle>('fast');
        ...
        {step === 1 ? (
          ...
          <div className="flex-1 flex flex-col gap-4 animate-in fade-in slide-in-from-right-4">
            ...
            <div className="space-y-2 flex-1 overflow-y-auto">
              <div className="p-3 bg-white/5 rounded-lg flex items-center justify-between border border-white/5">
                <span className="flex items-center gap-2 text-xs text-slate-300">
                  <Type size={14} /> تم توليد السكريبت
                </span>
              ...
              <div className="p-3 bg-white/5 rounded-lg flex items-center justify-between border border-white/5">
                <span className="flex items-center gap-2 text-xs text-slate-300">
                  <Music size={14} /> موسيقى خلفية (Copyright Free)
                </span>
                <span className="text-green-400 text-[10px]">تم</span>
              </div>
              ...
              <div className="p-3 bg-white/5 rounded-lg flex items-center justify-between border border-white/5">
                <span className="flex items-center gap-2 text-xs text-slate-300">
                  <Video size={14} /> لقطات B-Roll ذكية
                </span>
                <span className="text-green-400 text-[10px]">تم</span>
              </div>

              <div className="p-3 bg-white/5 rounded-lg flex items-center justify-between border border-white/5">
                <span className="text-xs text-slate-300">النمط المختار</span>
                <span className="text-pink-400 text-[10px] font-bold uppercase">{selectedStyle}</span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1 bg-white text-black font-bold gap-2">
          ...
          {loading ? (
            ...
            <div className="absolute inset-0 bg-slate-800">
              <Image
                ...
                alt="معاينة فيديو الريلز الناتج"
                fill
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-4 pb-12 bg-gradient-to-t from-black/80 to-transparent">
                <h4 className="text-white font-bold mb-1 drop-shadow-md">
                  {topic || '5 حقائق عن الفضاء 🪐'}
                </h4>
                <p className="text-white/80 text-xs mb-4">#ai #{selectedStyle} #monteerly</p>
                <div className="flex gap-2 text-white">
                  <div className="w-8 h-8 bg-slate-700/50 rounded-full flex items-center justify-center backdrop-blur">
                    <Heart size={14} />
                  </div>
                  <div className="w-8 h-8 bg-slate-700/50 rounded-full flex items-center justify-center backdrop-blur">
