  ...
  return (
    <div className="group bg-brand-surface rounded-2xl border border-gray-800 hover:border-brand-secondary/50 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
      <div className="h-40 bg-gray-900 relative flex items-center justify-center overflow-hidden">
        ...
        <PlayCircle
          size={48}
          className="text-gray-700 group-hover:text-brand-secondary group-hover:scale-110 transition-all duration-300 relative z-20"
        />
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-3">
          <span
            className={`text-[10px] px-2 py-1 rounded-lg border font-bold ${statusColors[status]}`}
          >
            {statusText[status]}
          </span>
          <button
        ...
        <div className="w-full bg-gray-800 h-1.5 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full ${
              status === 'completed' ? 'bg-brand-success' : 'bg-brand-secondary'
            }`}
