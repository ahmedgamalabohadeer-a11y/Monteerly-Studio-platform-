  ...
  const posts = [
    {
      ...
      time: 'منذ ساعتين',
      content:
        'ما رأيكم في آخر مشروع قمت به باستخدام Blender؟ حاولت دمج الـ 2D مع 3D. النقد البناء مطلوب! 🎨',
    ...
    },
    {
      ...
      time: 'منذ 5 ساعات',
      content:
        'هل يواجه أحد مشكلة في تصدير ProRes من النسخة الأخيرة لـ Premiere؟ الألوان تظهر باهتة.',
    ...
    <div className="space-y-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between items-center mt-3">
            <div className="flex gap-2">
              <button className="text-xs text-slate-400 hover:text-white bg-white/5 px-3 py-1.5 rounded-full">
                📷 صورة
              </button>
              <button className="text-xs text-slate-400 hover:text-white bg-white/5 px-3 py-1.5 rounded-full">
                🔗 رابط
      ...
      <div className="space-y-4">
        {posts.map((post, i) => (
          <div
            key={i}
            className="bg-slate-900 border border-white/10 rounded-xl p-5 hover:border-indigo-500/30 transition-colors"
          >
            ...
            {post.image && (
              <div className="mb-4 rounded-lg overflow-hidden border border-white/5 relative w-full max-h-80 h-80">
                <Image
                  src={post.image}
                  alt={`صورة مرفقة في منشور ${post.author}`}
                  fill
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {post.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] bg-indigo-500/10 text-indigo-300 px-2 py-1 rounded border border-indigo-500/20"
                >
                  #{tag}
                </span>
              ))}
