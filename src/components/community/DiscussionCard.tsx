...
export function DiscussionCard({
  ...
  return (
    <div className="bg-slate-900 border border-white/10 rounded-xl p-5 hover:border-indigo-500/30 transition-colors mb-4">
      ...
      {image && (
        <div className="mb-4 rounded-lg overflow-hidden border border-white/5 relative w-full min-h-[240px]">
          <Image
            src={image}
            alt={`مرفق المناقشة: ${title}`}
            fill
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {tags.map((tag) => (
          <span key={tag} className="px-2 py-1 bg-white/5 rounded text-[10px] text-indigo-300">
            #{tag}
          </span>
        ))}
      </div>
