...
import { Play, PenTool, MessageCircle, ChevronLeft, ThumbsUp, ThumbsDown } from 'lucide-react';
    ...
    <div className="bg-black h-[calc(100vh-80px)] flex flex-col relative overflow-hidden rounded-xl border border-white/10 max-w-sm mx-auto shadow-2xl">
      <div className="absolute top-0 left-0 w-full p-4 bg-gradient-to-b from-black/80 to-transparent z-20 flex justify-between items-start">
        <button
          ...
          className="text-white bg-black/20 backdrop-blur p-2 rounded-full"
        >
          <ChevronLeft size={20} />
        </button>
      ...
      <div className="flex-1 bg-slate-900 relative flex items-center justify-center">
        <Image
          ...
          alt="معاينة لقطة الفيديو قيد المراجعة"
          fill
          priority
      ...
      <div className="bg-slate-950 p-6 pb-8 rounded-t-2xl z-20 border-t border-white/10">
        <div className="flex justify-between items-center mb-6">
          <button
            type="button"
            aria-label={drawing ? 'إيقاف الرسم التوضيحي' : 'تفعيل الرسم التوضيحي'}
          ...
          <div className="flex gap-4">
            <button
              type="button"
              aria-label="رفض اللقطة"
              className="p-4 bg-red-500/10 text-red-500 rounded-full border border-red-500/20 hover:bg-red-500 hover:text-white transition-all"
            >
              <ThumbsDown size={24} />
            </button>
            <button
              type="button"
              aria-label="الموافقة على اللقطة"
              className="p-4 bg-green-500/10 text-green-400 rounded-full border border-green-500/20 hover:bg-green-500 hover:text-black transition-all"
            >
              <ThumbsUp size={24} />
            </button>
          </div>
