...
import { Play, Heart, MessageCircle, Share2, UserPlus, Briefcase } from 'lucide-react';
  ...
  return (
    <div className="relative w-full max-w-md mx-auto aspect-[9/16] bg-black rounded-2xl overflow-hidden shadow-2xl border border-white/10 mb-8 group">
      <div className="absolute inset-0 bg-slate-900">
        <Image
          src={data.videoSrc}
          alt={`معاينة أعمال ${data.creator.name}`}
          fill
      ...
      <div className="absolute right-4 bottom-24 flex flex-col gap-6 items-center z-20">
        <div className="flex flex-col items-center gap-1">
          <button
            ...
            className={`p-3 rounded-full bg-black/40 backdrop-blur hover:bg-black/60 transition-all ${
      ...
      <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black via-black/80 to-transparent p-6 pt-20 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-white text-sm flex items-center gap-2">
                {data.creator.name}
                {data.creator.available && (
                  <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="متاح للعمل" />
                )}
              </h3>
        ...
        <div className="flex flex-wrap gap-2">
          {data.tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-1 bg-white/10 backdrop-blur rounded-md text-[10px] text-white border border-white/10"
            >
              #{tag}
            </span>
          ))}
