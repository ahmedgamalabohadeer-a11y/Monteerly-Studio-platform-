      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-bold text-lg flex items-center gap-2">
            <Shield className="text-emerald-600" size={20} /> سجل النشاط الأمني (Audit
            Log)
          </h3>
          <p className="text-sm text-muted-foreground">
            تتبع جميع الإجراءات الحساسة التي تمت في مساحة العمل.
          </p>
        </div>
...
function LogRow({ user, event, details, ip, date, isRisk = false }: LogRowProps) {
  return (
      ...
      <td className="p-4">
        <span
          className={`px-2 py-1 rounded text-xs font-bold ${
            isRisk ? 'bg-red-100 text-red-700' : 'bg-blue-50 text-blue-700'
          }`}
        >
          {event}
        </span>
