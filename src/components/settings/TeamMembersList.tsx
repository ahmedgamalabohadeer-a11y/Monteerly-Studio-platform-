...
import { MoreHorizontal, User, Shield, Trash2, Mail } from 'lucide-react';
             ...
             <div key={member.id} className="flex items-center justify-between p-4 hover:bg-muted/20 transition-colors">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <h4 className="font-bold text-sm flex items-center gap-2">
                         {member.name}
                         {member.status === 'pending' && <Badge variant="secondary" className="text-[10px]">معلق</Badge>}
                      </h4>
                ...
                <div className="flex items-center gap-4">
                   <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted/50 border border-border text-xs font-bold">
                      {member.role === 'Owner' ? <Shield size={12} className="text-primary" /> : <User size={12} />}
                      {member.role}
                   </div>
