...
export function WebhooksManager() {
  const [endpoints] = useState([
    {
      id: 1,
      url: 'https://api.myagency.com/callbacks/monteerly',
      events: ['project.completed', 'payment.received'],
      status: 'active',
    },
  ]);
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            استقبل إشعارات برمجية فورية عند حدوث تغييرات في حسابك.
          </p>
        </div>
            ...
            <div className="flex justify-between items-start mb-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    {ep.events.map((ev) => (
                      <span
                        key={ev}
                        className="text-[10px] bg-muted px-2 py-0.5 rounded border border-border"
                      >
                        {ev}
                      </span>
                    ))}
              ...
              <div className="flex items-center gap-2">
                <span className="flex items-center gap-1 text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">
                  <Activity size={12} /> {ep.status === 'active' ? 'Active' : ep.status}
                </span>
