       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
          <div className="space-y-6">
             ...
             <p className="text-slate-400 leading-relaxed">
                Monteerly ليست تكلفة إضافية. هي أداة توفر عليك 40% من وقت المراسلات الضائع وتغنيك عن 5 اشتراكات مختلفة.
             </p>
             ...
             <div className="space-y-4 pt-4">
                <div>
                   ...
                   <input type="range" min="10" max="200" value={hours} onChange={(e) => setHours(Number(e.target.value))} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                </div>
                <div>
                   <label className="flex justify-between text-sm text-slate-300 mb-2">
                   ...
                   <input type="range" min="10" max="200" value={rate} onChange={(e) => setRate(Number(e.target.value))} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                </div>
             </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-xl p-8 border border-white/10 flex flex-col justify-center items-center text-center">
             <div className="text-slate-400 text-sm mb-2 font-medium">توفيرك السنوي المتوقع</div>
             <div className="text-5xl md:text-6xl font-black text-white mb-2 tracking-tight">
                {yearlySavings.toLocaleString()} <span className="text-2xl text-indigo-400">{currency}</span>
             </div>
