...
# إنشاء مسار المحفظة المالية
wallet_dir = "src/app/[locale]/dashboard/wallet"
file_path = os.path.join(wallet_dir, "page.tsx")
...
export default function WalletPage() {
  ...
  return (
    <div className="min-h-screen bg-[#05050A] text-slate-50 p-8" dir="rtl">
        <h1 className="text-3xl font-black mb-8 flex items-center gap-3"><Wallet className="text-indigo-500" /> المحفظة المالية السيادية</h1>
            ...
            {walletData.transactions.map(t => (
                <div key={t.id} className="flex justify-between items-center py-4 border-b border-white/5">
                    ...
                    <div className="text-right">
                        <p className={`font-black ${t.amount.startsWith('+') ? 'text-emerald-500' : 'text-slate-200'}`}>{t.amount}</p>
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("✅ تم إنشاء صفحة المحفظة المالية (Wallet)! الخطوة الأخيرة قبل الـ Dashboard.")
